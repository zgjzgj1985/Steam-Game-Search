"""
采集所有零评价游戏(扩展): 为有类型或标签的零评价游戏批量获取评价数
"""
...
BACKUP_FILE = Path(r'D:\Steam全域游戏搜索\public\data\games-index.json.ext_backup')
...
for appid, g in db.items():
    ...
    has_tag = has_tags(g)
    # OR逻辑: 只要有类型或标签之一即可
    if not (has_genre or has_tag):
        continue
    targets.append(appid)

total = len(targets)
log(f'    Target games (zero reviews, has genre OR tag): {total:,}')
